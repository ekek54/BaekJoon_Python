...
<p> 구현 문제이다. N이 최대 100이다.</p>
<p> N길이 짜리 경로가 2N개 존재한다. N짜리 경로를 O(N)에 건널 수 있는지 판단할 수 있기에 O(N^2)의 시간복잡도가 발생한다.</p>
<p> 경로가 건널 수 있는지 없는지 파악하는 방법은 문제의 조건을 적용시켜 경사로를 설치할 수 있는지 없는지 파악하며 경로를 탐색하면된다. </p>
<p> 경로를 한칸씩 탐색하며 다음칸을 확인해 높이 차이가 얼마나 나는지 확인한 후 경사로를 설치 할 수 있는지 확인한다. </p>
<p> 오르막의 경우 현재 칸부터 L칸이 동일한 높이인지 확인하고 경사로가 설치된적 있는지 확인하여 경사로를 설치한다. </p>
<p> 내리막의 경우 다음 칸부터 L칸이 동일한 높이인지 확인하고 경사로가 설치된적 있는지 확인하여 경사로를 설치한다. </p>